...
| --- | --- | --- | --- | --- |
| 矩阵matrix：A=[] | 返回一个 2×2 的初始化矩阵 | A=[1,2; 3,4] | {::nomarkdown}<div>arma::mat A={{1,2},{3,4}}</div> | $A=[1,2; 3,4]$|
...
| repelem(A, m, n) | 创建一个矩阵，并将A中每个元素复制到一个新矩阵的 m×n 块中。 | `A = [1 2; 3 4]; B = repelem(A,3,2);` | {::nomarkdown}<div>arma::mat A = {{1,2},{3,4}}; arma::mat B = arma::repelem(A,3,2);</div> | $A = [1 2; 3 4]; B = [1, 1, 2, 2; 1, 1, 2, 2; 1, 1, 2, 2; 3, 3, 4, 4; 3, 3, 4, 4; 3, 3, 4, 4;]$ |
